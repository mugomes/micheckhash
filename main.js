...
if (!fs.existsSync(path.join(sOS.userInfo().homedir, '/.micheckhash/'))) {
    fs.mkdirSync(path.join(sOS.userInfo().homedir, '/.micheckhash/'));
}

if (!fs.existsSync(path.join(sOS.userInfo().homedir, '/.micheckhash/config.json'))) {
    const config1 = JSON.parse('{ "app": { "disableAccelerationHardware": true } }');
    config1.app.disableAccelerationHardware = true;
    fs.writeFileSync(path.join(sOS.userInfo().homedir, '/.micheckhash/config.json'), JSON.stringify(config1, '', "\t"));
}

const config = JSON.parse(fs.readFileSync(path.join(sOS.userInfo().homedir, '/.micheckhash/config.json'), 'utf-8'));

if (config.app.disableAccelerationHardware) {
    app.disableHardwareAcceleration();
}

app.setName('MICheckHash');
